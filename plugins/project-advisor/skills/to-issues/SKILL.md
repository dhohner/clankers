...
In `default` mode, ask the user for the PRD source if it is not already in context. Prefer a PRD file that already exists in the workspace. Markdown PRDs and styled HTML PRDs generated by `to-prd` are both valid input. If needed, ask for the relevant file path or for the PRD text to be pasted.

If the PRD is already available in the workspace, read it directly.

When the source PRD is HTML, extract the planning content from the semantic section text and ignore presentational markup, inline CSS, metadata pills, comments, and other browser-review chrome. Treat the document sections and their text as the source of truth, not the surrounding layout. Use a structured parser when one is readily available; otherwise, read the HTML carefully enough to preserve section intent without copying template scaffolding into tickets.
