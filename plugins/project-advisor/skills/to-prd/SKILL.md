...
name: to-prd
description: >-
  PRD authoring and revision. Use when the user wants product decisions or feature requests turned into a validated requirements document, or wants review feedback folded into an existing PRD.
---
...
Bound the tree at PRD altitude.
...
Work the tree in **rounds** and use the interactive question tool.
When none exists, ask concise numbered chat questions.
...
After the user's answers, recompute the frontier and ask the next round.
...
A running exploration is an unsettled prerequisite: only its downstream questions wait for the sub-agent's report, so ask the rest of the frontier now.
...
- Retain German only for exact repository-backed identifiers, filenames, API names, product labels, or domain idioms.
  For retained German, attach evidence where its field supports it; otherwise quote it as repository terminology.
...
- Give every diagram a concise description and readable Mermaid `source` with meaningful node and edge labels.
  Include failure, fallback, decision, or boundary paths when they affect scope or acceptance.
