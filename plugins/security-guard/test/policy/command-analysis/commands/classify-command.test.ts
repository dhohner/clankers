...
describe("isDestructiveText", () => {
  ...
  it.each([
    ...
    // A second checkout operand is always a pathspec, `-f` discards local changes even when switching
    // branches, and `-B` resets an existing branch to the start point. A lone operand is decided with the
    // host's help; see the host path check cases below.
