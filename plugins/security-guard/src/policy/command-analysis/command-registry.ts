...
export type ClassificationOptions =
  ...
  | { kind: "chmod" | "chown"; recursiveShort: string; recursiveLong: readonly string[]; referenceLong: readonly string[] }
  | {
      kind: "git";
      leading: OptionModel;
...
const rules: readonly CommandRule[] = [
  {
    ...
    effect: {
      ...
      options: {
        kind: "standard",
        longFlags: ["--force", "--interactive", "--one-file-system", "--no-preserve-root", "--preserve-root", "--recursive", "--dir", "--verbose"],
        longValues: [],
  ...
  },
  {
    ...
    effect: {
      ...
      options: {
        ...
        shortFlags: "finuvbZT",
        shortValues: "tS",
        longFlags: new Set(["--force", "--interactive", "--no-clobber", "--update", "--verbose", "--backup", "--strip-trailing-slashes", "--no-target-directory", "--context"]),
  ...
  ...(["chmod", "chown"] as const).map((name): CommandRule => ({
    ...
    effect: {
      ...
      options: {
        kind: "mode",
        shortFlags: "fvchHPN",
        longFlags: new Set(["--force", "--silent", "--quiet", "--verbose", "--changes", "--no-dereference", "--preserve-root", "--no-preserve-root"]),
      },
      symlinkBehavior: "target",
      allowsShallowGlob: false,
    },
  ...
  })),
  {
    ...
    classificationOptions: {
      kind: "git",
      leading: {
        value: new Set(["-C", "-c", "--git-dir", "--work-tree", "--namespace", "--super-prefix", "--config-env", "--list-cmds", "--attr-source"]),
        flag: new Set(["-p", "--paginate", "-P", "--no-pager", "--bare", "--no-replace-objects", "--no-lazy-fetch", "--no-optional-locks", "--no-advice", "--literal-pathspecs", "--glob-pathspecs", "--noglob-pathspecs", "--icase-pathspecs", "--exec-path", "--html-path", "--man-path", "--info-path", "--version", "--help"]),
      },
  ...
  ...(["bash", "sh", "zsh", "ksh", "dash", "ash"] as const).map((name): CommandRule => ({
    ...
    classificationOptions: {
      ...
      flags: new Set(["--debugger", "--dump-po-strings", "--dump-strings", "--help", "--login", "--noediting", "--noprofile", "--norc", "--posix", "--pretty-print", "--restricted", "--verbose", "--version"]),
  ...
  },
  {
    ...
    operandCommandOptions: {
      value: new Set(["-a", "--arg-file", "-d", "--delimiter", "-E", "-I", "-J", "-L", "-n", "--max-args", "-P", "--max-procs", "-R", "-S", "-s", "--max-chars"]),
      flag: new Set(["-0", "--null", "-o", "--open-tty", "-p", "--interactive", "-r", "--no-run-if-empty", "-t", "--verbose", "-x", "--exit"]),
    },
  },
  {
    names: ["mkdir"],
  ...
  ...(["cat", "echo", "exit", "false", "grep", "head", "ls", "pwd", "sleep", "stat", "tail", "test", "true", "wc"] as const).map((name): CommandRule => ({
    names: [name],
    classification: "never",
    effect: { kind: "inert" },
  ...
  { names: ["printf"], classification: "never", effect: { kind: "inert", unsafeOption: /^-[A-Za-z]*v/ }, shellBuiltin: true },
  { names: ["diff"], classification: "never", effect: { kind: "inert", unsafeOption: /^(-[A-Za-z]*o|--o[a-z]*(=|$))/ } },
  { names: ["export"], classification: "never", effect: { kind: "none" }, shellBuiltin: true },
  { names: ["builtin"], classification: "never", effect: { kind: "none" }, wrapper: emptyOptions(), shellBuiltin: true },
  {
    names: ["command"],
    classification: "never",
    effect: { kind: "none" },
  ...
  { names: ["exec"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-a"]), flag: new Set(["-c", "-l"]) }, shellBuiltin: true },
  { names: ["setsid"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(), flag: new Set(["-c", "--ctty", "-f", "--fork", "-w", "--wait"]) } },
  { names: ["stdbuf"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-i", "--input", "-o", "--output", "-e", "--error"]), flag: new Set() } },
  { names: ["timeout"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-s", "--signal", "-k", "--kill-after"]), flag: new Set(["--preserve-status", "--foreground", "-v", "--verbose"]), operands: 1 } },
  { names: ["nohup"], classification: "never", effect: { kind: "none" }, wrapper: { ...emptyOptions(), alwaysStateful: true } },
  { names: ["nice"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-n", "--adjustment"]), flag: new Set(), numeric: true } },
  { names: ["time"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-o", "--output", "-f", "--format"]), flag: new Set(["-p", "--portability", "-a", "--append", "-v", "--verbose"]), stateful: new Set(["-o", "--output"]) }, shellBuiltin: true },
  {
    names: ["sudo"],
    classification: "never",
    effect: { kind: "none" },
    wrapper: {
      value: new Set(["-u", "--user", "-g", "--group", "-p", "--prompt", "-C", "--close-from", "-D", "--chdir", "-h", "--host", "-R", "--chroot", "-r", "--role", "-t", "--type", "-T", "--command-timeout", "-U", "--other-user"]),
      flag: new Set(["-A", "--askpass", "-b", "--background", "-E", "--preserve-env", "-H", "--set-home", "-K", "--remove-timestamp", "-k", "--reset-timestamp", "-l", "--list", "-n", "--non-interactive", "-P", "--preserve-groups", "-S", "--stdin", "-V", "--version", "-v", "--validate", "-e", "--edit"]),
      stateful: new Set(["-D", "--chdir", "-R", "--chroot"]),
    },
    escalatesPrivilege: true,
  },
  { names: ["doas"], classification: "never", effect: { kind: "none" }, wrapper: { value: new Set(["-C", "-u"]), flag: new Set(["-L", "-n"]) }, escalatesPrivilege: true },
  {
    names: ["env"],
    classification: "never",
    effect: { kind: "none" },
    wrapper: { value: new Set(["-u", "--unset", "-C", "--chdir"]), flag: new Set(["-i", "-0", "--ignore-environment", "--null"]), stateful: new Set(["-C", "--chdir"]) },
  },
];
